        ...
        <a-form :model="formState"
          ...
          <template v-if="step ===1">
            <a-form-item label="密码">
              <a-input v-model:value="formState.password" placeholder="密码"></a-input>
            </a-form-item>
            <a-form-item label="验证码">
              <a-input v-model:value="formState.code" placeholder="请输入邮箱验证码"></a-input>
            </a-form-item>
          </template>
        </a-form>
      </div>
...
let validateEmail = async (_rule, value) => {
  if (/^\w+@[a-z0-9]+\.[a-z]+$/i.test(formState.email)) {
...
async function updateUserInfo(column) {
  ...
  const data = {}
  data[column] = val
  let res = await updateUserIngoApi(data)
  if (res.code) {
